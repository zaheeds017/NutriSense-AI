/**
 * Food-Science Rule Engine (client-side port of NutriScan engine)
 * Computes ingredient health flags, 1-10 health score, UK/EU FSA traffic lights,
 * dietary compatibility, and allergen detection entirely in the browser.
 */
...
/**
 * Standardize a raw ingredients string into a clean list.
 */
...
/**
 * Identify the EU-14 + US major allergens present in an ingredients list.
 */
  ...
  for (const allergen of ALLERGEN_DICTIONARY) {
    for (const kw of allergen.keywords) {
      ...
      if (regex.test(fullText)) {
        if (!identified.includes(allergen.name)) identified.push(allergen.name);
        break;
      }
    }
  }
...
/**
 * Compute UK/EU FSA traffic-light colors for macros (per 100g / serving thresholds).
 */
export function computeTrafficLights(macros = {}) {
...
/**
 * Evaluate dietary restriction compatibility for a list of ingredients + macros.
 */
export function evaluateDietaryFlags(ingredients = [], macros = {}, allergens = []) {
...
/**
 * Main rule-based analysis engine → HealthData contract.
 */
export function analyzeProductRules(productName, brand, ingredientsList = [], macros = {}, servingInfo = {}, userPrefs) {
  const ingredientsWithFlags = [];
  let scorePoints = 7.0;
  ...
  for (const ing of ingredientsList) {
    ...
    for (const h of HARMFUL_INGREDIENTS) {
      if (h.pattern.test(ing)) {
        ingredientsWithFlags.push({ name: ing, status: 'harmful', color: 'red', reason: h.reason, categoryTag: h.tag });
        scorePoints -= 1.2;
        matched = true;
        break;
      }
    ...
    for (const b of BENEFICIAL_INGREDIENTS) {
      if (b.pattern.test(ing)) {
        ingredientsWithFlags.push({ name: ing, status: 'good', color: 'green', reason: b.reason, categoryTag: b.tag });
        scorePoints += 0.8;
        matched = true;
        break;
      }
    ...
    for (const a of AMBER_INGREDIENTS) {
      if (a.pattern.test(ing)) {
        ingredientsWithFlags.push({ name: ing, status: 'neutral', color: 'amber', reason: a.reason, categoryTag: a.tag });
        scorePoints -= 0.4;
        matched = true;
        break;
      }
  ...
  const dietaryFlags = evaluateDietaryFlags(ingredientsList, macros, allergens);

  // Personal conflict flags derived from the user profile
  const personalFlags = [];
  (userPrefs?.allergens || []).forEach((pref) => {
    const p = String(pref).toLowerCase();
    const match = allergens.find(
      (a) => a.toLowerCase() === p || a.toLowerCase().includes(p) || p.includes(a.toLowerCase())
    );
    if (match) {
      personalFlags.push({ type: 'allergen', value: pref, matched: match, severity: 'critical' });
      scorePoints -= 1.5;
    }
  });
  (userPrefs?.dietRestrictions || []).forEach((diet) => {
    const flag = dietaryFlags.find((df) => df.diet.toLowerCase() === diet.toLowerCase());
    if (flag && !flag.suitable) {
      personalFlags.push({ type: 'diet', value: diet, matched: null, reason: flag.reason, severity: 'warning' });
      scorePoints -= 0.5;
    }
  });
  ...
  if (personalFlags.some((f) => f.severity === 'critical')) {
    recommendations.push(`WARNING: This product conflicts with your listed allergies (${personalFlags.filter((f) => f.severity === 'critical').map((f) => f.value).join(', ')}). Avoid or verify with the manufacturer.`);
  }
  ...
  const servingNote = servingInfo?.serving_size ? ` Evaluated per serving (${servingInfo.serving_size}).` : '';
  ...
  } beneficial ingredient(s).${servingNote}`;
...
/**
 * Enrich a validated schema data object (serving_info, macros, ingredients,
 * allergens_identified) with the full health analysis contract.
 */
export function enrichWithHealth(data = {}, productName, brand, userPrefs) {
  const health = analyzeProductRules(
    productName || data.product_name || 'Parsed Food Product',
  ...
  return {
    ...data,
    health,
    product_name: data.product_name || productName || 'Parsed Food Product',
    brand: data.brand || brand || 'Generic',
  };
}
